...
class _DaftarGuruKuesionerPageState extends State<DaftarGuruKuesionerPage> with SingleTickerProviderStateMixin {
  List listGuru = [];
  List _filtered = [];
...
class _GuruRow extends StatelessWidget {
  ...
  const _GuruRow({required this.no, required this.nama, required this.jabatan, required this.nip, required this.mapel, required this.sudahDinilai, required this.isLast, required this.onTap});
  static const Color kPrimary = Color(0xFF1A5FA8);
    ...
    return Column(
      children: [
        Padding(
          ...
          child: Row(
            children: [
              SizedBox(width: 40, child: Text('$no', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF6B7A99)))),
              Expanded(
                flex: 3,
                child: Row(
                  children: [
                    CircleAvatar(radius: 20, backgroundColor: kPrimary.withOpacity(0.10), child: const Icon(Icons.person_rounded, color: kPrimary, size: 21)),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(nama, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: Color(0xFF1A2340)), overflow: TextOverflow.ellipsis),
                          if (jabatan.isNotEmpty) Text(jabatan, style: const TextStyle(fontSize: 11, color: Color(0xFF9AABCC)), overflow: TextOverflow.ellipsis),
                        ],
                      ),
                    ),
                  ],
              ...
              Expanded(flex: 2, child: Text(nip, style: const TextStyle(fontSize: 12, color: Color(0xFF4A5568)), overflow: TextOverflow.ellipsis)),
              Expanded(flex: 2, child: Text(mapel, style: const TextStyle(fontSize: 12, color: Color(0xFF4A5568)), overflow: TextOverflow.ellipsis)),
              Expanded(
                flex: 2,
                child: Row(
                  children: [
                    Icon(sudahDinilai ? Icons.check_circle_rounded : Icons.hourglass_empty_rounded, size: 15, color: sudahDinilai ? const Color(0xFF2E7D32) : const Color(0xFFF57F17)),
                    const SizedBox(width: 5),
                    Text(sudahDinilai ? 'Sudah Dinilai' : 'Belum Dinilai', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: sudahDinilai ? const Color(0xFF2E7D32) : const Color(0xFFF57F17))),
                  ],
                ),
              ),
              Expanded(
                  ...
                  children: [
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(color: sudahDinilai ? kPrimary.withOpacity(0.08) : kPrimary, borderRadius: BorderRadius.circular(8), border: sudahDinilai ? Border.all(color: kPrimary.withOpacity(0.3)) : null),
                        child: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Icon(sudahDinilai ? Icons.visibility_rounded : Icons.person_add_alt_1_rounded, size: 14, color: sudahDinilai ? kPrimary : Colors.white),
                            const SizedBox(width: 5),
                            Text(sudahDinilai ? 'Lihat Detail' : 'Pilih Guru', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: sudahDinilai ? kPrimary : Colors.white)),
                          ],
                        ),
                      ),
                    ),
