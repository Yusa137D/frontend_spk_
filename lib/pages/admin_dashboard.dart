...
class _AdminDashboardState extends State<AdminDashboard> {
    ...
    return Row(
      ...
      children: [
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text("Dashboard Admin", style: TextStyle(fontSize: 22, fontWeight: FontWeight.w800, color: _textPrimary)),
        ...
        Container(
          ...
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            const Padding(padding: EdgeInsets.only(right: 8.0), child: AdminEasterEgg()),
            CircleAvatar(radius: 16, backgroundColor: const Color(0xFF2563EB), child: Text(widget.user['username'][0].toUpperCase(), style: const TextStyle(color: Colors.white, fontSize: 13))),
    ...
    return Column(
      ...
      children: [
        ...
        Wrap(spacing: 16, runSpacing: 16, children: [
          _menuBox("Lihat Ranking", Icons.leaderboard_rounded, const Color(0xFFF59E0B), const Color(0xFFFFFBEB), () => Navigator.push(context, MaterialPageRoute(builder: (_) => const RankingPage()))),
          _menuBox("Cetak Laporan", Icons.picture_as_pdf_rounded, const Color(0xFF10B981), const Color(0xFFECFDF5), () => html.window.open("http://127.0.0.1:5000/api/print-pdf", "_blank")),
          _menuBox("Kelola Guru", Icons.manage_accounts_rounded, const Color(0xFFEF4444), const Color(0xFFFEF2F2), () => Navigator.push(context, MaterialPageRoute(builder: (_) => const KelolaGuruPage()))),
          _menuBox("Kelola Siswa", Icons.people_alt_rounded, const Color(0xFF8B5CF6), const Color(0xFFF5F3FF), () => Navigator.push(context, MaterialPageRoute(builder: (_) => const KelolaSiswaPage()))),
        ]),
      ],
    );
  }
  ...
  Widget _menuBox(String title, IconData icon, Color color, Color bg, VoidCallback onTap) {
    return GestureDetector(onTap: onTap, child: Container(width: 155, padding: const EdgeInsets.symmetric(vertical: 22, horizontal: 16), decoration: BoxDecoration(color: _cardBg, borderRadius: BorderRadius.circular(18), boxShadow: [BoxShadow(color: Colors.grey.shade100, blurRadius: 12, offset: const Offset(0, 4))]), child: Column(children: [Container(width: 52, height: 52, decoration: BoxDecoration(color: bg, borderRadius: BorderRadius.circular(14)), child: Icon(icon, color: color)), const SizedBox(height: 12), Text(title, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13))])));
  }
}